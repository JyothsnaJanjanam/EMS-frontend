...
const LeaveDetail = () => {
  ...
  useEffect(() => {
    const fetchLeave = async () => {
      try {
        const response = await axios.get(`https://ems-backend-one.vercel.app/api/leave/details/${id}`, {
          headers: {
  ...
  const changeStatus = async (id, status) => {
    try {
      const response = await axios.put(`https://ems-backend-one.vercel.app/api/leave/${id}`, {status}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('auth-token')}`
        }
      })
    ...
    <>{leave ? (
      <div className='view-detail-display'>
        ...
        <div className='view-detail-cont'>
          <div>
            <img src={`https://ems-backend-one.vercel.app/${leave.employeeId.userId.profileImage}`} className='view-detail-img' />
          </div>
          <div>
